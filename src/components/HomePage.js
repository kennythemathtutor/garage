...
const contract = new Contract(
  "0x2beaF3FEeA2f521fFD151DF0935c789C45dBeb71",
  abi,
  provider
);
  ...
  return (
    <div className="min-h-screen bg-yellow-400">
      <div className="max-w-7xl mx-auto sm:px-6 xl:px-8 ">
        <div className="text-gray-900 text-6xl pt-28 pb-10">POKEMON</div>
          ...
          <div className="grid grid-cols-3 gap-x-1">
            {mintedNftState.data.map(
              ({ id, image, name, description, owner }) => {
                return (
                  <div key={id} className="bg-yellow-400 hover:bg-green-400 rounded p-2">
                    <img src={image} className="mx-auto p-1 rounded" alt={name} />
                    <div className="text-xl">{name}</div>
                    <div className="">{description}</div>
                    <hr className=" my-3" />
                    <div className="text-center text-sm">Owned By:</div>
        ...
        <div className="mt-12 ">
          <button
            ...
            type="button"
            className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-blue-700 hover:text-yellow-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          >
            Get Your Starter Pokemon Here
          </button>
        </div>
      </div>
      {modalVisible && (
